...
pub fn tracker_matches_layout(ct: &CollisionTracker, l: &Layout) -> bool {
    ...
    for (pk1, pi1) in &l.placed_items {
        ...
        assert!((ct.get_pair_loss(pk1, pk1) - 0.0).abs() <= f32::EPSILON);
        ...
        if collector.contains_entity(&HazardEntity::Exterior) {
            ...
            assert!((ct.get_container_loss(pk1) - 0.0).abs() <= f32::EPSILON);
        }
    }
...
pub fn custom_pipeline_matches_jaguars(shape: &SPolygon, det: &SpecializedHazardCollector) -> bool {
    ...
    let default_set: HashSet<HazardEntity> = base_detector.entities().copied().collect();
    let custom_set: HashSet<HazardEntity> = det.entities().copied().collect();
